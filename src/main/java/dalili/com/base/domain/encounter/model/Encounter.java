...
public class Encounter {
    ...
    /**
     * Family history captured during physician encounter.
     */
    @Column(length = 2000)
    ...
    /**
     * Whether transcript was explicitly verified by physician.
     * Background-attached transcripts are marked as not physician-verified.
     */
    @Column
    private boolean transcriptPhysicianVerified = false;

    /**
     * Processing state for ambient transcription pipeline.
     */
    @Enumerated(EnumType.STRING)
    @Column
    private AmbientTranscriptionStatus ambientTranscriptionStatus = AmbientTranscriptionStatus.NOT_REQUESTED;

    /**
     * Timestamp when ambient transcription was queued.
     */
    @Column
    private Instant ambientTranscriptionQueuedAt;

    /**
     * Timestamp when ambient transcription processing finished.
     */
    @Column
    private Instant ambientTranscriptionCompletedAt;

    /**
     * Failure detail for ambient transcription, if any.
     */
    @Column(length = 1000)
    private String ambientTranscriptionError;
    ...
    /**
     * Cancellation reason captured when encounter is cancelled.
     */
    @Column(length = 1000)
    private String cancellationReason;

    /**
     * Timestamp when encounter was cancelled.
     */
    @Column
    private Instant cancelledAt;

    /**
     * Staff ID that cancelled the encounter.
     */
    @Column
    ...
    public void recordTranscript(String transcript) {
        ...
        this.transcriptPhysicianVerified = true;
        this.ambientTranscriptionStatus = AmbientTranscriptionStatus.COMPLETED;
        this.ambientTranscriptionCompletedAt = Instant.now();
        this.ambientTranscriptionError = null;
        this.status = EncounterStatus.DOCUMENTING;
    }

    /**
     * Records transcript generated by background ambient pipeline.
     *
     * <p>Used when encounter recording is ended and transcription is finalized asynchronously.
     * This path intentionally marks transcript as not physician-verified.</p>
     */
    public void recordBackgroundTranscript(String transcript) {
        if (this.status == EncounterStatus.CANCELLED) {
            throw new IllegalStateException("Cannot attach transcript to cancelled encounter");
        }
        if (this.transcript != null) {
            throw new IllegalStateException("Transcript already recorded. Cannot overwrite.");
        }
        ...
        this.transcript = transcript;
        this.transcriptRecordedAt = Instant.now();
        this.transcriptPhysicianVerified = false;
        this.ambientTranscriptionStatus = AmbientTranscriptionStatus.COMPLETED;
        this.ambientTranscriptionCompletedAt = Instant.now();
        this.ambientTranscriptionError = null;
        if (this.status == EncounterStatus.OPEN) {
            this.status = EncounterStatus.DOCUMENTING;
        }
    }

    /**
     * Marks ambient transcription request as queued.
     */
    public void queueAmbientTranscription() {
        if (this.status == EncounterStatus.CANCELLED) {
            throw new IllegalStateException("Cannot queue transcription for cancelled encounter");
        }
        this.ambientTranscriptionStatus = AmbientTranscriptionStatus.PENDING;
        this.ambientTranscriptionQueuedAt = Instant.now();
        this.ambientTranscriptionError = null;
    }

    /**
     * Marks ambient transcription as actively processing.
     */
    public void markAmbientTranscriptionProcessing() {
        if (this.status == EncounterStatus.CANCELLED) {
            throw new IllegalStateException("Cannot process transcription for cancelled encounter");
        }
        this.ambientTranscriptionStatus = AmbientTranscriptionStatus.PROCESSING;
        this.ambientTranscriptionError = null;
    }

    /**
     * Marks ambient transcription as failed with a reason.
     */
    public void markAmbientTranscriptionFailed(String errorMessage) {
        if (this.status == EncounterStatus.CANCELLED) {
            return;
        }
        this.ambientTranscriptionStatus = AmbientTranscriptionStatus.FAILED;
        this.ambientTranscriptionCompletedAt = Instant.now();
        this.ambientTranscriptionError = errorMessage;
    }

    /**
     * Marks ambient transcription as completed when transcript attachment is intentionally skipped.
     */
    public void markAmbientTranscriptionCompletedWithoutAttachment() {
        if (this.status == EncounterStatus.CANCELLED) {
            return;
        }
        this.ambientTranscriptionStatus = AmbientTranscriptionStatus.COMPLETED;
    ...
    /**
     * Stores transcript quality assessment generated by the system.
     * This can run after delayed background transcription completes.
     */
    public void applySystemTranscriptAssessment(
            NoteAccuracyRating rating,
            Integer score,
            String discrepancySummary
    ) {
        if (rating == null) {
            throw new IllegalArgumentException("Transcript assessment rating is required");
        }
        this.aiAccuracyRating = rating;
        this.transcriptAccuracyScore = score;
        this.transcriptDiscrepancySummary = discrepancySummary;
    ...
    /**
     * Records family history for the encounter.
     */
    public void recordFamilyHistory(String history) {
        assertNotCompleted();
        assertNoteNotConfirmed();
    ...
    /**
     * Ambient AI transcription processing status.
     */
    public enum AmbientTranscriptionStatus {
        NOT_REQUESTED,
        PENDING,
        PROCESSING,
        COMPLETED,
